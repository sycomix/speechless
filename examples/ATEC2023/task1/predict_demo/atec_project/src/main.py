...
def main():
    ...
    if training_args.do_predict:
        ...
        print_test_dataset_example(predict_dataset[0])
    ...
    if training_args.do_predict:
        ...
        if trainer.is_world_process_zero():
            if training_args.predict_with_generate:
                ...
                output_prediction_file = os.path.join(training_args.output_dir, "predictions.jsonl")
                with open(output_prediction_file, "w", encoding="utf-8") as writer:
                    for p, l in zip(predictions, predict_dataset_raw):
                        res = json.dumps({"id": l["id"], "question": l["question"], "answer": p}, ensure_ascii=False)
                        writer.write(f"{res}\n")
    return results
