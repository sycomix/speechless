...
# ---------- Open-Orca/OpenOrca ----------
def load_orca_dataset(train_data_path: str, do_filter=True):
...
def prepare_data(model_name_or_path, model_max_len):
    ...
    # speechless-orca.jsonl
    dataset_file = "/opt/local/datasets/Speechless/speechless-orca.jsonl"
    orca_dataset = load_orca_dataset("/opt/local/datasets/OpenOrca", do_filter=False)

    # > 32768:     134
    # > 16384:   2,811 ( 2,677)
    # > 8192 :  14,138 (11,327) 
    # > 4096 :  44,340 (30,202) 
    # > 2048 : 267,305 (222,965) 20000
    # > 1024 : 554,892 (287,587) 15000
    # > 512 :  812,393 (257,501) 5000
    # <= 512 : 182,503 (141,788)
    # <= 256 :  40,715
    orca_dataset_4096 = orca_dataset.filter(lambda x: len(x['instruction'] + x['response']) >= 4096 and len(x['instruction'] + x['response']) < 32768 )

    orca_dataset_2048 = orca_dataset.filter(lambda x: len(x['instruction'] + x['response']) >= 2048 and len(x['instruction'] + x['response']) < 4096 )
    orca_dataset_2048 = orca_dataset_2048.train_test_split(test_size=20000)['test']

    orca_dataset_1024 = orca_dataset.filter(lambda x: len(x['instruction'] + x['response']) >= 1024 and len(x['instruction'] + x['response']) < 2048 )
    orca_dataset_1024 = orca_dataset_1024.train_test_split(test_size=15000)['test']

    orca_dataset_512 = orca_dataset.filter(lambda x: len(x['instruction'] + x['response']) >= 512 and len(x['instruction'] + x['response']) < 1024 )
    orca_dataset_512 = orca_dataset_512.train_test_split(test_size=5000)['test']

    dataset = datasets.concatenate_datasets(
      [orca_dataset_4096, orca_dataset_2048, orca_dataset_1024, orca_dataset_512])
