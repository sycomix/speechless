...
def prepare_data(model_name_or_path, model_max_len):
    ...
    # # speechless-reasoning-v0.3-tiny.jsonl
    # dataset_file = "/opt/local/datasets/Speechless/speechless-reasoning-v0.3-tiny.jsonl"
    # airoboros_dataset = load_airoboros_dataset()
    # orca_dataset = load_orca_dataset("/opt/local/datasets/OpenOrca")
    # platypus_dataset = load_platypus_dataset("/opt/local/datasets/garage-bAInd/Open-Platypus")
    # wizardlm_dataset = load_wizardlm_dataset("/opt/local/datasets/WizardLM/WizardLM_evol_instruct_V2_196k")
    ...
    # airoboros_dataset = airoboros_dataset.train_test_split(sampling_ratio)['test']
    # orca_dataset = orca_dataset.train_test_split(sampling_ratio)['test']
    # platypus_dataset = platypus_dataset.train_test_split(sampling_ratio)['test']
    # wizardlm_dataset = wizardlm_dataset.train_test_split(sampling_ratio)['test']
    # tinycodes_dataset = tinycodes_dataset.train_test_split(sampling_ratio)['test']

    # dataset = datasets.concatenate_datasets(
    #   [airoboros_dataset, orca_dataset, platypus_dataset, wizardlm_dataset, tinycodes_dataset])

    # speechless-reasoning-v0.3-small.jsonl
    dataset_file = "/opt/local/datasets/Speechless/speechless-reasoning-v0.3-small.jsonl"
    airoboros_dataset = load_airoboros_dataset()
    orca_dataset = load_orca_dataset("/opt/local/datasets/OpenOrca")
    platypus_dataset = load_platypus_dataset("/opt/local/datasets/garage-bAInd/Open-Platypus")
    wizardlm_dataset = load_wizardlm_dataset("/opt/local/datasets/WizardLM/WizardLM_evol_instruct_V2_196k")
...
def main():
    model_name_or_path="/opt/local/llm_models/huggingface.co/mistralai/Mistral-7B-v0.1"
    model_max_len = 4096
