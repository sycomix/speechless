...
def load_agent_instruct_dataset():
    ...
    def _format(sample):
        ...
        return {
            ...
            "toolllama",
            "system_prompt":
            "A chat between a curious user and an artificial intelligence assistant who can use external tools and APIs to solve the user's question."
...
def load_airoboros_dataset():
    ...
    airoboros_dataset = datasets.load_dataset(
        "json", data_files="/opt/local/datasets/base/jondurbin/airoboros-2.2.1/instructions.jsonl"
    )['train']
    ...
    print(
        f"Filter airoboros dataset from {total_samples} to {remaining_samples}. {remaining_samples/total_samples * 100:.2f}%"
    )
...
def load_airoboros_22_dataset():
    ...
    airoboros_dataset = datasets.load_dataset(
        "json", data_files="/opt/local/datasets/base/jondurbin/airoboros-2.2/instructions.jsonl"
    )['train']
    ...
    airoboros_dataset = airoboros_dataset.map(lambda x: {
        'category': f"airoboros.{x['category']}",
    })
...
def load_orca_dataset(train_data_path: str):
    ...
    print(
        f"Filter orca dataset from {total_samples} to {remaining_samples}. {remaining_samples/total_samples * 100:.2f}%"
    )
...
def load_wizardlm_dataset(train_data_path: str):
    ...
    remaining_samples = len(ds)
    print(
        f"Filter wizardlm dataset from {total_samples} to {remaining_samples}. {remaining_samples/total_samples * 100:.2f}%"
    )
...
def load_tinycodes_dataset(train_data_path: str, n_samples=-1):
    """
    ...
    selected_langs = ['Python', 'Java', 'JavaScript', 'C++', 'Rust', 'Go']

    ds_list = []
    for lang in selected_langs:
        print(f"promgramming language: {lang}")
        lang_ds = ds.filter(lambda x: x['programming_language'] == lang)
        print(f"{len(lang_ds)} {lang} samples")
        if n_samples > 0:
            print(f"{len(lang_ds)} {lang} samples. Selecting {n_samples} samples ...")
            lang_ds = lang_ds.train_test_split(test_size=n_samples)['test']
...
# ---------- OpenHermes ----------
def load_openhermes_dataset(train_data_path: str):
    print("Loading OpenHermes ...")
    ds = datasets.load_dataset("json", data_files=train_data_path)['train']
    ds = ds.filter(lambda x: '```' in x['output'])

    ds = ds.rename_column('output', 'response')
    ds = ds.remove_columns(['input'])

    ds = ds.add_column('category', ['openhermes'] * len(ds))

    return ds


# ---------- CollectiveCognition ----------
def load_collective_cognition_dataset(train_data_path: str):
    print("Loading CollectiveCognition ...")
    ds = datasets.load_dataset("json", data_files=train_data_path)['train']
    conversions = ds['conversations']
    instructions = []
    responses = []
    ...
    ds = datasets.Dataset.from_dict({
        ...
        'response': responses,
        'category': ['CollectiveCognition'] * len(instructions)
    })
...
def load_magicoder_oss_dataset(train_data_path: str):
    ...
    def _filter_fn(samples):
        ...
        outputs = {
            ...
            'response': solutions,
            'category': [f'magicoder.{lang}' for lang in langs],
        }
...
def load_magicoder_evol_dataset(train_data_path: str):
    print("Loading Magicoder evol-instruct ...")
    ds = datasets.load_dataset("json", data_files=train_data_path)['train']

    ds = ds.add_column('category', ['magicoder-evol'] * len(ds))

    return ds

# ---------- meta-math/MetaMathQA ----------
def load_metamath_dataset(train_data_path: str):
    ...
    # ds = ds.map(_filter_fn, batched=True, remove_columns=ds.column_names)
    metamath_cats = {
        'GSM_AnsAug': 16000, #80000,
        'GSM_Rephrased': 16000, #80000,
        'MATH_AnsAug': 15000, #75000,
        'MATH_Rephrased': 10000, #50000,
        'GSM_SV': 8000, #40000,
        'GSM_FOBAR': 8000, # 40000,
        'MATH_SV': 3000, # 15000,
        'MATH_FOBAR': 3000, #15000
    }
    ds_c_list = []
    for i, c in enumerate(metamath_cats):
        ds_c = ds.filter(lambda x: x['type'] == c)
        ds_c = ds_c.train_test_split(test_size=metamath_cats[c])['test']
        ds_c_list.append(ds_c)
    ds = datasets.concatenate_datasets(ds_c_list)

    def _filter_fn(samples):
        instructions = samples['query']
        responses = samples['response']

        outputs = {
            'instruction': instructions,
            'response': responses,
            'category': ['MetaMath'] * len(instructions),
        }
...
def prepare_data(model_name_or_path, model_max_len, model_min_len):
    ...
    dataset_file = "/opt/local/datasets/speechless_data/speechless-magicoder-evol-dataset.jsonl"
    ...
    # openhermes_dataset = load_openhermes_dataset("/opt/local/datasets/base/OpenHermes/openhermes.json")

    # collective_cognition_dataset = load_collective_cognition_dataset("/opt/local/datasets/base/CollectiveCognition/chats_data2023-10-16.json")

    # magicoder_oss_dataset = load_magicoder_oss_dataset("/opt/local/datasets/base/Magicoder/data-oss_instruct-decontaminated.jsonl")

    magicoder_evol_dataset = load_magicoder_evol_dataset("/opt/local/datasets/base/Magicoder/data-evol_instruct-decontaminated.jsonl")

    # metamath_dataset = load_metamath_dataset("/opt/local/datasets/base/MetaMathQA/MetaMathQA-395K.json")

    # not use tinycodes
    # tinycodes_dataset = load_tinycodes_dataset("/opt/local/datasets/code/nampdn-ai/tiny-codes", n_samples=-1) #25000)
    # tinycodes_dataset = tinycodes_dataset.cast(collective_cognition_dataset.features)
    # tinycodes_dataset = tinycodes_dataset.train_test_split(test_size=10000)['test']

    # dataset = datasets.concatenate_datasets([
    #     airoboros_dataset, orca_dataset, platypus_dataset, wizardlm_dataset, tokenbender_dataset, collective_cognition_dataset, magicoder_dataset 
    # ])
    dataset = magicoder_evol_dataset
    ...
    dataset = dataset.remove_columns(
        ...
        [
            ...
            # if col not in ['conversations', 'system_prompt', 'category', 'prompt_type']
...
def main():
    model_name_or_path = "/opt/local/llm_models/huggingface.co/llm-agents/tora-code-7b-v1.0"
